...
tppiDeviceAACSource*
tppiDeviceAACSource::createNew(UsageEnvironment& env,
			tppiDeviceAACParameters params) {
  return new tppiDeviceAACSource(env, params);
}

EventTriggerId tppiDeviceAACSource::eventTriggerId = 0;

unsigned tppiDeviceAACSource::referenceCount = 0;

tppiDeviceAACSource::tppiDeviceAACSource(UsageEnvironment& env,
			   tppiDeviceAACParameters params)
  : FramedSource(env), fParams(params), fuSecsPerFrame(1000000/params.fps) {
  if (referenceCount == 0) {
...
tppiDeviceAACSource::~tppiDeviceAACSource() {
...
void tppiDeviceAACSource::doGetNextFrame() {
...
void tppiDeviceAACSource::deliverFrame0(void* clientData) {
  ((tppiDeviceAACSource*)clientData)->deliverFrame();
}

void tppiDeviceAACSource::deliverFrame() {
...
// to make "eventTriggerId" a non-static member variable of "tppiDeviceAACSource".)
void signalNewAACFrameData() {
  TaskScheduler* ourScheduler = NULL; //%%% TO BE WRITTEN %%%
  tppiDeviceAACSource* ourDevice  = NULL; //%%% TO BE WRITTEN %%%

  if (ourScheduler != NULL) { // sanity check
    ourScheduler->triggerEvent(tppiDeviceAACSource::eventTriggerId, ourDevice);
  }
}
